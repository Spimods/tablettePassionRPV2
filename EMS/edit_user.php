...
if (count($_POST) > 0) {
    $sql = "UPDATE usersems set signup_name=?, userName=? ,firstName=? ,lastName=? WHERE userId=?";
    $paramType = 'ssssi';
    $paramValue = array(
        $_POST["signup-name"],
        $_POST["userName"],
...
.home {
	...
	<?PHP 
require_once __DIR__ . '/lib/DataSource.php';
$sql = "select * from usersems where userId=? ";
$paramType = 'i';
$paramValue = array(
    $_GET["userId"]
);
...
<form name="frmUser" method="post" action="">
    ...
    <div>
        <div class="row">
            <label for="signup-name">Nom <span
    ...
    <div class="row">
        <label>Prénom</label> <input type="hidden"
    ...
    <div class="row">
        <label>Facture</label> <input type="text"
