...
$sql = "SELECT * FROM `fiche` WHERE userId=?";
$paramType = 'i';
$paramValue = array(
    $_GET["userId"]
);
...
$name_user = $result[0]['signup_name'];
$database = new DataSource();
$sql = "SELECT * FROM dépôts WHERE nom = ?";
$paramType = 's';
$paramValue = array(
    $name_user
);

$result2 = $database->select($sql,$paramType, $paramValue);
...
.interior{
    margin-left: 0.5em;
    margin-right: 0.5em;
    border-bottom: solid 1px #72D5CA ;
}
b{
    margin-left: 1em;
    color: #579f96;
...
if (is_array($result) || is_object($result)) {
    foreach ($result as $key => $value) {
        ?>
			...
			<a><?php echo $result[$key]["signup_name"];?>
            <?php echo $result[$key]["userName"];?></a>

 <?php
    }
}
?></div><br>
<div class="total">Total : <?php echo $result[$key]["firstName"];?>$</div>
<div class="fiche">Nom : <?php echo $result[$key]["signup_name"];?><br>
Prénom : <?php echo $result[$key]["userName"];?><br>
Téléphone : <?php echo $result[$key]["lastName"];?></div>
</div>
...
if (is_array($result2) || is_object($result2)) {
    foreach ($result2 as $key => $value) {
        ?>
<div class="interior"> 
<div class="date"><?php echo $result2[$key]["date"];?> <b><?php echo $result2[$key]["prix"];?>$</b></div>
<div class="bobo"><?php echo $result2[$key]["element"];?></div>
</div>
<?php
    }
}
...
<div class="home" onclick="window.location='citoyen.php'"></div>
